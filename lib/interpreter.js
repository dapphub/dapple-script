...
var TYPE = {
  ...
  ADDRESS: {atom: true, toString: () => {return "address"}},
  //
  //                          Polytype
  //
...
class Interpreter {
  ...
  constructor (opts) {
    ...
    _.each(env.objects, (value, key) => {
      ...
      switch (typeof value) {
        ...
        case 'object': // asume contract object
          type = new TYPE.OBJECT(value.class);
          value = value.address;
          break;
        default:
    ...
    this.web3Interface._web3.eth.getBlockNumber((err, bn) => {
      this.log(`Connected to RPC client, block height is ${bn}\n`);
    })
  }
  ...
  static interpret(scope, ast) {
    ...
    if (!(ast instanceof Expr)) {// VALUE
      ...
      if(ast.value.name === 'ret') {
        return ast.value.apply(scope, ast.args);
      } else {
  ...
  export (key) {
    if (key in this.global) {
      ...
      // console.log(this.local, this.global);
  ...
  fcall (name) {
    if(! name in this.local) throw new Error(`Unknown function "${name}" called!`);
    // console.log(name);
    // console.log(this.local[name].toString());
    switch (this.local[name].type.toString()) {
      case "Expr":
      case "Seq":
        Interpreter.interpret(this, this.local[name]);
        break;
      default:
        throw new Error('Unhandled typecall, please report to github issues what you was thinking!');
    }
  }
