...
module.exports = {
  ...
  performStaticCall: function (runState, co, localOpts, cb) {
    runState.ds.web3Interface._web3.eth.call(co, (err, result) => {
      ...
      var rtcode = ctx.getCode(co.to).slice(2);
      ...
      var result = '';
      if(db.fabi) {
        params = runState.ds.coder.decodeParams(db.fabi.inputs.map(i => i.type), co.data.slice(10)).map(p => p.toString()).join(', ');
        result = runState.ds.coder.decodeParams(db.fabi.outputs.map(i => i.type), _return.toString('hex')).map(p => p.toString())
  ...
  getBalance: function (runState, address) {
    return deasync(runState.ds.web3Interface._web3.eth.getBalance.bind(runState.ds.web3Interface._web3.eth, address))();
  }
}
