...
class Expert {
  constructor () {
    ...
    this.address_to_chaintype = {};
  }

  learn(classes) {
    _.each(classes, (json, name) => {
      let classId = sha3(json.bytecode);
      let rtcodeId = sha3(json.runtimeBytecode);
  ...
  learnAddress(addr, classId, chaintype) {
    this.address_to_contract[addr.toLowerCase()] = this.classid_to_contract[classId];
    this.address_to_chaintype[addr.toLowerCase()] = chaintype;
  }
  ...
  from(ctx) {
    ...
    if('code' in ctx) {
      this.sources.forEach((code, name) => {
