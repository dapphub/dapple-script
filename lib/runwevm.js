...
var runwevm = (opts, cb) => {
  ...
  _.each(libs, (lib, address) => expert.learnAddress(address, lib.classid));

  _.each(opts.chainenv.env, (obj, name) => {
    if( obj.type.indexOf('[') == -1) return null;
    ...
    if(type in opts.classes > -1 && classid in expert.classid_to_contract) {
      expert.learnAddress(obj.value, classid);
    }
  });
...
//     console.log(`OPCODE: ${data.opcode.name} PC: ${data.pc}`);
