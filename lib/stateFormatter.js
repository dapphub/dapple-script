...
class StateFormatter {
  ...
  getReport () {
    var report = this.steps.map((step, i) => {
      ...
      var msg = '';
      var status = '';
      // var isInteraction = type === `NEW` || type === `TXR`;
      // var isCurrentStep = i === this.currentStep;
      // var isPreviousStep = i < this.currentStep;
      // if(isInteraction && isCurrentStep) status = clc.cyanBright('>');
      // if(isInteraction && isPreviousStep) status = clc.greenBright('✔');
      switch (type) {
        case 'NEW':
